...
def linear_drift(ss):
    ...
    for a0 in tqdm(range(len(ss.linearSearch))):
        for a1 in tqdm(range(len(ss.linearSearch))):
            xyShift = [ss.inds*ss.xDrift[a0, a1],
                       ss.inds*ss.yDrift[a0, a1]]
...
def SPmerge01(data, scanAngles):
    ...
    ss.linearSearchScore1 = np.zeros(
        (len(ss.linearSearch), len(ss.linearSearch)))
...
# First linear
    ...
    # Second linear
        ...
        xRefine = ss.linearSearch[xInd] + \
            np.linspace(-0.5, 0.5, len(ss.linearSearch))*step
        yRefine = ss.linearSearch[yInd] + \
            np.linspace(-0.5, 0.5, len(ss.linearSearch))*step

        ss.yDrift, ss.xDrift = np.meshgrid(yRefine, xRefine)
        ss.linearSearchScore2 = np.zeros(
            (len(ss.linearSearch), len(ss.linearSearch)))
        with open('test.txt', 'a') as f:
            f.write('Linear2\n')
        ss = linear_drift(ss)
    ...
    ss.xyLinearDrift = np.array(
        [ss.xDrift[xInd, 0], ss.yDrift[yInd, 0]])
    ...
    for a0 in range(1, ss.numImages):
        ...
        m = G1*np.conj(G2)
        Icorr = np.fft.ifft2(
            np.sqrt(abs(m))*np.exp(1j*np.angle(m))
            ).real
